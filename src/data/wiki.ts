export type WikiDoc = {
  meta: {
    ...
    maintainers: { name: string; contact: string }[];
  };
  summary: {
    one_liner: string;
  ...
  architecture: {
    ...
    callouts: { title: string; bullets: string[] }[];
  };
  ...
  operational_readiness: {
    ...
    runbooks: { title: string; summary: string }[];
  };
  getting_started: {
    dev_setup: string[];
    sample_usage: string;
...
export type WikiStore = Record<string, WikiDoc>;

export const PREPROCESSED_WIKIS: WikiStore = {
  "https://github.com/facebook/rocksdb": {
    ...
    summary: {
      ...
      architecture_overview:
        "Log-structured merge-tree (LSM) engine with layered caches and pluggable filesystems. Core abstractions: Env (IO), Table Formats (SST), MemTable (in-memory), VersionSet (manifest), and Compaction.",
    },
    architecture: {
      components: [
        {
    ...
    getting_started: {
      ...
      sample_usage:
        "DB* db; Options opts; opts.create_if_missing = true; Status s = DB::Open(opts, \"/tmp/testdb\", &db);",
    },
    faq: [
      {
